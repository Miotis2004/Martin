...
This standalone Martin alpha sample demonstrates optional types, `nil`, and `if let` conditional binding.
...
Both present and absent optional paths are fixed. The sample has no external dependencies, performs no network access, and does not rely on repository-relative paths.
