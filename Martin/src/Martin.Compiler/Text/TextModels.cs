...
public sealed class SourceText
{
    ...
    private SourceText(string text, string? filePath)
    {
        _text = text;
        FilePath = filePath;
        Lines = ParseLines(this, text).ToImmutableArray();
    }
    ...
    public LinePosition GetLinePosition(int position)
    {
        ...
        var line = GetLineIndex(clamped);
        return new(line + 1, Math.Max(0, clamped - Lines[line].Start));
    }
    ...
    private static List<TextLine> ParseLines(SourceText source, string text)
    {
        ...
        {
            var lb = GetLineBreakWidth(text, position);
            if (lb == 0)
            {
                position++;
                continue;
            }
            AddLine(result, source, lineStart, position, lb);
            position += lb;
            lineStart = position;
        }
        if (position >= lineStart)
            AddLine(result, source, lineStart, position, 0);
        return result;
    }
...
public readonly record struct TextLocation(SourceText Text, TextSpan Span)
{
    ...
    public LinePosition StartLinePosition => Text.GetLinePosition(Span.Start);
    public LinePosition EndLinePosition => Text.GetLinePosition(Span.End);
