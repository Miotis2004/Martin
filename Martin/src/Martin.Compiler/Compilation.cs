...
public sealed class Compilation
{
    ...
    {
        var validator = new GenericConstraintValidator(attempts.Select(a => a.Validated).OfType<ProtocolConformance>());
        foreach (var construction in _genericConstructions)
        {
            cancellationToken.ThrowIfCancellationRequested();
            foreach (var pair in construction.Definition.TypeParameters.Zip(construction.Arguments))
            {
                foreach (var constraint in validator.Validate(pair.First, pair.Second).UnsatisfiedConstraints.OfType<ProtocolConstraint>())
                {
                    diagnostics.Report(new("MRT2304", DiagnosticSeverity.Error, $"Type argument '{pair.Second.Name}' does not satisfy protocol constraint '{constraint.Protocol.Name}' for type parameter '{pair.First.Name}'.", construction.Location));
                }
            }
        }
        foreach (var construction in _genericCallableConstructions)
        {
            cancellationToken.ThrowIfCancellationRequested();
            foreach (var pair in construction.Parameters.Zip(construction.Arguments))
            {
                foreach (var constraint in validator.Validate(pair.First, pair.Second).UnsatisfiedConstraints.OfType<ProtocolConstraint>())
                {
                    diagnostics.Report(new("MRT2304", DiagnosticSeverity.Error, $"Type argument '{pair.Second.Name}' does not satisfy protocol constraint '{constraint.Protocol.Name}' for type parameter '{pair.First.Name}'.", construction.Location));
    ...
    {
        if (string.Equals(function.Name, "main", StringComparison.Ordinal) && function.IsThrowing)
            d.Report(new("MRT2192", DiagnosticSeverity.Error, "Entry point 'main' cannot be throwing.", function.Locations.FirstOrDefault()));
        ValidatePropagation(function.IsThrowing, function.ErrorType, body, function.Locations.FirstOrDefault(), d, cancellationToken);
    }
    static void ValidatePropagation(MethodSymbol method, BoundBlockStatement body, DiagnosticBag d, CancellationToken cancellationToken) => ValidatePropagation(method.IsThrowing, method.ErrorType, body, method.Locations.FirstOrDefault(), d, cancellationToken);
    static void ValidatePropagation(InitializerSymbol initializer, BoundBlockStatement body, DiagnosticBag d, CancellationToken cancellationToken) => ValidatePropagation(initializer.IsThrowing, initializer.ErrorType, body, initializer.Locations.FirstOrDefault(), d, cancellationToken);
    static void ValidatePropagation(bool isThrowing, TypeSymbol? declaredErrorType, BoundStatement body, TextLocation fallbackLocation, DiagnosticBag d, CancellationToken cancellationToken)
    {
        ...
        {
            d.Report(new("MRT2402", DiagnosticSeverity.Error, "Expression or body contains incompatible error effects.", location));
            if (isThrowing && declaredErrorType is not null)
                d.Report(new("MRT2199", DiagnosticSeverity.Error, $"Callable declares error type '{declaredErrorType}', but the residual effect contains incompatible error types.", location));
            else
    ...
    static (ErrorEffect Effect, TextLocation? Location) AggregateAcknowledgedEffect(BoundStatement statement, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        switch (statement)
        {
