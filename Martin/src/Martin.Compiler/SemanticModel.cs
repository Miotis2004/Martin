    ...
    public GenericSemanticInfo? GetGenericInfo(SyntaxNode node)
    {
        ...
        while (semanticNode is not null)
        {
            ...
            if (_symbols.TryGetValue(semanticNode, out symbol) &&
                symbol is ConstructedTypeSymbol or InitializerSymbol { ContainingType : ConstructedTypeSymbol } or EnumCaseSymbol { ContainingType : ConstructedTypeSymbol })
                break;
        ...
        if (genericUse is not null)
        {
            ...
            case ConstructedTypeSymbol type:
                original = type.GenericDefinition;
                arguments = type.TypeArguments;
                typeIdentity = type.Identity;
                argumentsWereInferred = false;
                break;
            case InitializerSymbol { ContainingType:
        ...
        argumentsWereInferred = false;
        break;
    case EnumCaseSymbol { ContainingType:
        ConstructedTypeSymbol type
    }:
                    original = type.GenericDefinition;
    symbol = type;
...
public SwitchAnalysisResult? GetSwitchAnalysis(StatementSyntax switchStatement)
{
    ...
    if (switchStatement.Kind != SyntaxKind.SwitchStatement || !IsInThisTree(switchStatement))
        return null;
...
public PatternSemanticInfo? GetPatternInfo(PatternSyntax pattern)
{
    ...
    if (!IsInThisTree(pattern))
        return null;
    var bound = _compilation.FindPattern(new TextLocation(SyntaxTree.Text, pattern.Span));
    return bound is null ? null : new(bound.InputType, bound.DeclaredVariables, bound is BoundEnumCasePattern enumCase ? enumCase.Case : null, bound.IsIrrefutable, bound.HasErrors);
}
...
public ImmutableArray<SymbolReference> GetSymbolReferences()
{
    ...
    foreach (var (syntax, info) in _typedErrorUses.Where(pair => IsInThisTree(pair.Key)))
    {
        ...
        if (errorType is null || ReferenceEquals(errorType, TypeSymbol.Error))
            continue;
        if (_symbols.TryGetValue(syntax, out var existing) && ReferenceEquals(CanonicalSymbol(existing), errorType))
            continue;
...
public TypeSymbol? GetExpectedType(int position)
{
    ...
    for (var current = node; current is not null; current = _parents.GetValueOrDefault(current))
    {
        if (current.Kind == SyntaxKind.ReturnStatement)
            return enclosing switch { FunctionSymbol f => f.ReturnType, MethodSymbol m => m.ReturnType,
                                      _ => null };
...
public ImmutableArray<Symbol> GetInvocationCandidates(SyntaxNode invocationOrIncompleteInvocation)
{
    ...
    if (callee is GenericExpressionSyntax { Kind : SyntaxKind.MemberAccessExpression } memberAccess)
    {
        ...
        token = memberAccess.GetChildren().OfType<SyntaxToken>().FirstOrDefault(candidate => candidate.Kind == SyntaxKind.IdentifierToken);
    }
    else
    {
        token = Descendants(callee).Prepend(callee).OfType<SyntaxToken>().FirstOrDefault(candidate => candidate.Kind == SyntaxKind.IdentifierToken);
    }
    if (token is null)
        return [];
...
private T? AncestorOrSelf<T>(SyntaxNode? node)
    where T : SyntaxNode
{
    while (node is not null)
    {
...
{
    if (declaration.Span.Start > position)
        return false;
    if (declaration is MemberSyntax)
        return true;
    ...
    if (declarationOwner is not null && !ReferenceEquals(declarationOwner, positionOwner))
        return false;
    ...
    for (var block = positionBlock; block is not null; block = AncestorOfKind(_parents.GetValueOrDefault(block), SyntaxKind.BlockStatement))
        if (ReferenceEquals(block, declarationBlock))
            return true;
    return false;
}
...
private NamedTypeSymbol? GetContainingType(SyntaxNode? node)
{
    while (node is not null)
    {
        if (_declared.TryGetValue(node, out var symbol) && symbol is NamedTypeSymbol type)
            return type;
        node = _parents.GetValueOrDefault(node);
    }
    return null;
...
private TextLocation DeclarationLocation(Symbol symbol, SyntaxNode syntax) =>
    symbol.DeclarationLocation is
{
}
location ? location : new TextLocation(SyntaxTree.Text, syntax.Span);
...
private SymbolReferenceKind ReferenceKind(SyntaxNode syntax, Symbol symbol)
{
    ...
    if (symbol is TypeSymbol)
        return SymbolReferenceKind.Type;
    ...
    if (symbol is MemberSymbol)
        return SymbolReferenceKind.MemberAccess;
