...
{
    readonly CSharpNameMangler _names = new();
    GeneratedSourceWriter _w = null!;
    LoweredProgram _program = null!;
    CodeGenerationRequest _request = null!;
    int _catchTemporary;
    public CodeGenerationResult Emit(CodeGenerationRequest request)
    {
        var diags = ImmutableArray.CreateBuilder<Diagnostic>();
        if (request.Program.Diagnostics.Any(d => d.Severity == DiagnosticSeverity.Error))
        {
            diags.Add(new("MRT3012", DiagnosticSeverity.Error, "Internal code-generation failure: semantic diagnostics are present.", request.Program.Diagnostics[0].Location));
            return new() { Diagnostics = diags.ToImmutable() };
        }
        if (request.OutputKind != OutputKind.ConsoleApplication)
        {
            diags.Add(NoLoc("MRT3014", $"Unsupported output kind '{request.OutputKind}'."));
            return new() { Diagnostics = diags.ToImmutable() };
        }
        var entry = request.Program.EntryPoint;
        if (entry is null)
        {
            diags.Add(NoLoc("MRT3001", "Executable target requires a valid main function."));
            return new() { Diagnostics = diags.ToImmutable() };
        }
        try
        {
            ...
            _catchTemporary = 0;
            WriteProgram(entry);
            return new() { Success = true, GeneratedSource = _w.ToString(), Diagnostics = [], SourceMap = _w.SourceMap };
        }
        catch (Exception ex)
        {
            return new() { Diagnostics = [NoLoc("MRT3012", $"Internal code-generation failure: {ex.Message}")] };
        }
    }
    ...
    void WriteProgram(FunctionSymbol entry)
    {
        _w.WriteLine("#nullable enable");
        if (_request.IncludeSourceDirectives)
            _w.WriteLine("#line hidden");
        _w.WriteLine();
        _w.WriteLine("namespace Martin.Generated;");
        _w.WriteLine();
        foreach (var t in DeterministicOrder.NamedTypes(_program.NamedTypes))
            WriteNamedType(t);
        _w.WriteLine("internal static class MartinProgram");
        _w.WriteLine("{");
        using (_w.Indent())
        {
            foreach (var f in _program.LoweredFunctions)
                WriteFunction(f);
            WriteRuntimeBoundaryHelpers();
        }
        _w.WriteLine("}");
        _w.WriteLine();
        _w.WriteLine("internal static class Program");
        _w.WriteLine("{");
        using (_w.Indent())
        {
            _w.WriteLine("public static int Main(string[] args)");
            _w.WriteLine("{");
            using (_w.Indent())
            {
                _w.WriteLine("Martin.Runtime.MartinConsole.SetArguments(args);");
                _w.WriteLine($"MartinProgram.{_names.GetName(entry)}();");
                _w.WriteLine("return 0;");
            }
            _w.WriteLine("}");
        }
        _w.WriteLine("}");
    }
    void WriteNamedType(NamedTypeSymbol t)
    {
        if (t is ProtocolTypeSymbol protocol)
        {
        ...
        _w.WriteLine($"internal {(t.IsStruct ? "struct" : "sealed class")} {_names.GetName(t)}{TypeParameterList(t)}{InterfaceList(t)}{GenericConstraints(t.TypeParameters)}");
        _w.WriteLine("{");
        using (_w.Indent())
        {
            foreach (var property in DeterministicOrder.Properties(t.Properties))
                _w.WriteLine($"internal {(property.IsReadOnly ? "readonly " : "")}{TypeName(property.Type)} {_names.GetName(property)};");
            foreach (var init in DeterministicOrder.Initializers(t.Initializers))
                WriteInitializer(init);
            foreach (var m in DeterministicOrder.Methods(t.Methods))
                WriteMethod(m);
            foreach (var conformance in ConformancesFor(t))
                WriteConformanceBridges(conformance);
        }
        _w.WriteLine("}");
        _w.WriteLine();
    }
    ...
    string InterfaceList(NamedTypeSymbol t)
    {
        var ps = ConformancesFor(t).Select(c => _names.GetName(c.Protocol)).ToArray();
        return ps.Length == 0 ? string.Empty : " : " + string.Join(", ", ps);
    }
    ...
    void WriteProtocol(ProtocolTypeSymbol p)
    {
        _w.WriteLine($"internal interface {_names.GetName(p)}");
        _w.WriteLine("{");
        using (_w.Indent())
        {
            foreach (var r in DeterministicOrder.ProtocolRequirements(p.Requirements))
                WriteProtocolRequirement(r);
        }
        _w.WriteLine("}");
        _w.WriteLine();
    }
        ...
        WriteSourceMapped(witness.Locations.FirstOrDefault(), () =>
                                                              {
                                                                  ...
                                                                  using (_w.Indent())
                                                                  {
                                                                      if (requirement.RequiresGetter)
                                                                          _w.WriteLine($"get => this.{_names.GetName(witness)};");
                                                                      if (requirement.RequiresSetter)
                                                                          _w.WriteLine($"set => this.{_names.GetName(witness)} = value;");
                                                                  }
                                                                  _w.WriteLine("}");
                                                              });
    }
    ...
    {
        if (location is not {} mappedLocation)
        {
            write();
            return;
        }
        if (_request.IncludeSourceDirectives)
            WriteLineDirective(mappedLocation);
    ...
    void WriteInitializer(InitializerSymbol init)
    {
        _w.Write($"internal {_names.GetName(init.ContainingType)}(");
        _w.Write(Parameters(init.Parameters));
        _w.WriteLine(")");
        _w.WriteLine("{");
        using (_w.Indent())
        {
            if (init.IsSynthesized)
            {
                ...
                {
                    var prop = ((NamedTypeSymbol)init.ContainingType).Properties.First(x => x.Name == p.Name);
                    _w.WriteLine($"{_names.GetName(prop)} = {_names.GetName(p)};");
                }
            }
            else
                foreach (var s in _program.InitializerBodies[init].Statements)
                    WriteStatement(s);
        }
        _w.WriteLine("}");
    }
    ...
    {
        _w.Write($"internal {TypeName(m.ReturnType)} {_names.GetName(m)}{TypeParameterList(m.TypeParameters)}(");
        _w.Write(Parameters(m.Parameters));
        _w.WriteLine($"){GenericConstraints(m.TypeParameters)}");
        _w.WriteLine("{");
        using (_w.Indent())
        {
            foreach (var s in _program.MethodBodies[m].Statements)
                WriteStatement(s);
        }
        _w.WriteLine("}");
    }
    string InterfaceMethodName(MethodSymbol m) => _names.GetName(m);
    void WriteFunction(LoweredFunction f)
    {
        _w.Write($"internal static {TypeName(f.Symbol.ReturnType)} {_names.GetName(f.Symbol)}{TypeParameterList(f.Symbol)}(");
        _w.Write(Parameters(f.Symbol.Parameters));
        _w.WriteLine($"){GenericConstraints(f.Symbol.TypeParameters)}");
        _w.WriteLine("{");
        using (_w.Indent())
        {
    ...
    void WriteStatement(BoundStatement s)
    {
        if (s is BoundBlockStatement)
        {
            WriteMappedStatement(s);
            return;
        }
        if (s.Location is {} loc)
        {
            if (_request.IncludeSourceDirectives)
                WriteLineDirective(loc);
            using (_w.MapTo(loc)) WriteMappedStatement(s);
            if (_request.IncludeSourceDirectives)
                _w.WriteLine("#line hidden");
            return;
        }
    ...
    void WriteMappedStatement(BoundStatement s)
    {
        switch (s)
        {
        ...
        case BoundVariableDeclaration v:
            _w.WriteLine($"{TypeName(v.Variable.Type)} {_names.GetName(v.Variable)} = {Expr(v.Initializer)};");
            break;
        case BoundExpressionStatement e:
            _w.WriteLine($"{Expr(e.Expression)};");
            break;
        case BoundIfStatement i:
            _w.WriteLine($"if {Condition(i.Condition)}");
            WriteStatement(i.ThenStatement);
            if (i.ElseStatement != null)
            {
                _w.WriteLine("else");
                WriteStatement(i.ElseStatement);
            }
            break;
        case BoundLabelStatement l:
            _w.WriteLine($"{l.Label}: ;");
            break;
        case BoundGotoStatement g:
            _w.WriteLine($"goto {g.Label};");
            break;
        case BoundConditionalGotoStatement g:
            _w.WriteLine($"if {Condition(g.Condition)} goto {g.Label};");
            break;
        case BoundIfLetStatement il:
            throw new CodeGenerationException("IfLet should have been lowered");
        ...
        case BoundReturnStatement r:
            _w.WriteLine(r.Expression == null ? "return;" : $"return {Expr(r.Expression)};");
            break;
        case BoundThrowStatement t:
            WriteThrow(t);
            break;
        case BoundDoCatchStatement dc:
            WriteDoCatch(dc);
            break;
        default:
            throw new CodeGenerationException($"Unsupported bound node '{s.Kind}'", s);
        }
    }
    ...
    void WriteLineDirective(TextLocation location)
    {
        var path = location.FilePath ?? string.Empty;
        path = path.Replace("\\", "\\\\").Replace("\"", "\\\"");
        _w.WriteLine($"#line {location.StartLinePosition.Line} \"{path}\"");
    }
    void WriteSwitch(BoundSwitchStatement sw)
    {
        _w.WriteLine($"switch ({Expr(sw.Expression)})");
        _w.WriteLine("{");
        using (_w.Indent())
        {
            foreach (var c in sw.Cases)
            {
                ...
                {
                    if (c.PatternVariable is null)
                        throw new CodeGenerationException("Lowered switch case is missing its pattern variable.");
                    _w.WriteLine($"case {TypeName(c.Case.ContainingType)}.{_names.GetName(c.Case)} {_names.GetName(c.PatternVariable)}:");
                }
                using (_w.Indent())
                {
                    if (c.Body is BoundBlockStatement cb)
                    {
                        foreach (var x in cb.Statements)
                            WriteStatement(x);
                    }
                    else
                        WriteStatement(c.Body);
                    _w.WriteLine("break;");
                }
            }
        }
