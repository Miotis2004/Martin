...
{
    readonly Dictionary<Symbol, string> _names = new(ReferenceEqualityComparer.Instance);
    readonly HashSet<string> _used = [];
    readonly TemporaryAllocator _temporaries = new();
    ...
    {
        if (_names.TryGetValue(symbol, out var n))
            return n;
        var prefix = symbol switch { FunctionSymbol f => f.Name == "main" ? "__martin_" : "__fn_", NamedTypeSymbol => "", EnumCaseSymbol => "", PropertySymbol => "", ProtocolRequirementSymbol => "", MethodSymbol => "__method_",
                                     _ => "__local_" };
        var baseName = Sanitize(symbol.Name);
        n = Unique(prefix + baseName);
        _names[symbol] = n;
        return n;
    }
    ...
    static string Sanitize(string s)
    {
        ...
        {
            var ch = s[i];
            b.Append((i == 0 ? char.IsLetter(ch) || ch == '_' : char.IsLetterOrDigit(ch) || ch == '_') ? ch : '_');
        }
        var r = b.ToString();
        return Keywords.Contains(r) ? "@" + r : r;
    }
}
