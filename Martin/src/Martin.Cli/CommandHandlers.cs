...
internal sealed class MartinCliApplication
{
    ...
    void RenderParserErrors(IEnumerable<string> errors)
    {
        ...
        {
            var code = IsOptionConflict(error) ? "MRT4508" : error.StartsWith("Unknown command", StringComparison.Ordinal) ? "MRT4501"
                                                         : error == "Project name is required."                            ? "MRT4506"
                                                                                                                           : "MRT4502";
...
internal sealed class CommandDiagnosticService
{
    ...
    public CommandDiagnosticService(IMartinConsole console, HumanDiagnosticRenderer renderer)
    {
    ...
    {
        DiagnosticRenderer selected = format == DiagnosticFormat.Json ? new JsonDiagnosticRenderer() : UseColor(format, color) ? new HumanDiagnosticRenderer(useColor: true)
                                                                                                                               : renderer;
...
internal interface IProjectLoaderService
{
    ProjectLoadResult Load(string? projectPath, string? manifestPath, CancellationToken cancellationToken = default);
}
...
internal sealed class CliBuildService : ICliBuildService
{
    ...
    public CliBuildService(IVersionProvider versions, IMartinBuildService buildService)
    {
    ...
    public async Task<BuildResult> BuildProjectAsync(MartinProject project, string outputDirectory, BuildConfiguration configuration, bool keepGeneratedFiles, bool useAppHost, bool emitPortablePdb, CancellationToken cancellationToken)
    {
        ...
        return await buildService.BuildAsync(Compilation.Create(trees), new BuildOptions { AssemblyName = project.Manifest.Package.Name, OutputDirectory = outputDirectory, Configuration = configuration, TargetFramework = project.Manifest.Target.Framework, KeepGeneratedFiles = keepGeneratedFiles, UseAppHost = useAppHost, EmitPortablePdb = emitPortablePdb, BuildStateInputs = new ProjectBuildStateInputs { ProjectRoot = project.RootDirectory, ManifestPath = project.ManifestPath, SourceFiles = project.SourceFiles.ToImmutableArray(), CompilerVersion = versions.MartinCompilerVersion, RuntimeVersion = versions.MartinRuntimeVersion } }, cancellationToken);
    }
}
...
internal sealed class VersionCommandHandler(IMartinConsole console, IVersionProvider versions)
{
    public int Handle(VersionCommandOptions _)
    {
        console.Out.WriteLine($"Martin CLI: {versions.MartinCliVersion}");
        console.Out.WriteLine($"Martin compiler: {versions.MartinCompilerVersion}");
        console.Out.WriteLine($"Martin runtime: {versions.MartinRuntimeVersion}");
        console.Out.WriteLine($"Language version: {versions.LanguageVersion}");
        console.Out.WriteLine($"Manifest version: {versions.ManifestVersion}");
        console.Out.WriteLine($"Target framework: {versions.TargetFramework}");
        return (int)MartinExitCode.Success;
    }
}
...
internal sealed class NewCommandHandler(IMartinConsole console, CommandDiagnosticService diagnostics, MartinProjectCreator creator)
{
    public int Handle(NewCommandOptions options)
    {
        ...
        if (!result.Success)
        {
            diagnostics.Render(result.Diagnostics);
            return (int)MartinExitCode.InvalidArguments;
        }
...
internal sealed class BuildCommandHandler(IMartinConsole console, CommandDiagnosticService diagnostics, IProjectLoaderService projects, ICliBuildService build)
{
    public async Task<int> HandleAsync(BuildCommandOptions options, CancellationToken cancellationToken)
    {
        ...
        if (!loaded.Success)
        {
            diagnostics.Render(loaded.Diagnostics, options.DiagnosticFormat, options.Color);
            return (int)(loaded.Diagnostics.Any(d => d.Code == "MRT4001") ? MartinExitCode.ProjectNotFound : MartinExitCode.ManifestInvalid);
        }
        var p = loaded.Project!;
        var config = options.Configuration;
        var outDir = options.Output ?? Path.Combine(p.RootDirectory, p.Manifest.Build.Output, config.ToString(), p.Manifest.Target.Framework);
        if (ShouldWriteStatus(options))
            console.Out.WriteLine($"Building {p.Manifest.Package.Name} {p.Manifest.Package.Version}");
        ...
        if (!res.Success)
        {
            diagnostics.Render(res.Diagnostics, options.DiagnosticFormat, options.Color);
            return (int)ExitCodeMapper.ForBuildFailure(res.Diagnostics);
        }
        if (ShouldWriteStatus(options))
            console.Out.WriteLine($"Build succeeded\n  Output: {res.OutputDirectory}");
        return (int)MartinExitCode.Success;
    }
...
internal sealed class RunCommandHandler(IMartinConsole console, CommandDiagnosticService diagnostics, IProjectLoaderService projects, ICliBuildService build, IMartinExecutionService execution, IVersionProvider versions)
{
    public async Task<int> HandleAsync(RunCommandOptions options, CancellationToken cancellationToken)
    {
        ...
        {
            diagnostics.Render(loaded.Diagnostics, options.DiagnosticFormat, options.Color);
            return (int)MartinExitCode.ProjectNotFound;
        }
        var p = loaded.Project!;
        var config = options.Configuration;
        var outDir = Path.Combine(p.RootDirectory, p.Manifest.Build.Output, config.ToString(), p.Manifest.Target.Framework);
        BuildResult br;
        if (options.NoBuild)
        {
            cancellationToken.ThrowIfCancellationRequested();
            ...
            br = await build.BuildProjectAsync(p, outDir, config, false, true, true, cancellationToken);
        if (!br.Success)
        {
            diagnostics.Render(br.Diagnostics, options.DiagnosticFormat, options.Color);
            return (int)MartinExitCode.BuildFailed;
        }
        ...
        console.Out.Write(er.StandardOutput);
        console.Error.Write(er.StandardError);
...
internal sealed class FormatCommandHandler(IMartinConsole console, CommandDiagnosticService diagnostics, IProjectLoaderService projects, MartinLanguageService formatter)
{
    public int Handle(FormatCommandOptions options)
    {
        ...
        if (files.Count == 0)
        {
            ...
            if (!loaded.Success)
            {
                diagnostics.Render(loaded.Diagnostics);
                return (int)MartinExitCode.ProjectNotFound;
            }
            files = loaded.Project!.SourceFiles.Order(StringComparer.Ordinal).ToArray();
        }
        if (options.Stdout && files.Count != 1)
        {
            console.Error.WriteLine("MRT4602: --stdout requires exactly one source file.");
            return (int)MartinExitCode.InvalidArguments;
        }
        ...
        foreach (var full in fullPaths)
        {
            if (!File.Exists(full))
            {
                console.Error.WriteLine($"MRT4601: Source file '{full}' does not exist.");
                return (int)MartinExitCode.InvalidArguments;
            }
        }
        ...
        foreach (var full in fullPaths)
        {
            ...
            var result = formatter.GetFormattingEdits(text);
            if (result.WasRefused)
            {
                console.Error.WriteLine($"{result.DiagnosticCode}: {result.Message} ({full})");
                return (int)MartinExitCode.CompilationFailed;
            }
            ...
            if (text != formatted)
            {
                changed = true;
                if (options.Check)
                    console.Out.WriteLine($"Would format {full}");
                else
                {
                    var temp = full + ".tmp." + Guid.NewGuid().ToString("N");
                    File.WriteAllText(temp, formatted);
                    File.Move(temp, full, overwrite: true);
                    console.Out.WriteLine($"Formatted {full}");
                }
            }
        }
...
internal sealed class CleanCommandHandler(IMartinConsole console, CommandDiagnosticService diagnostics, IProjectLoaderService projects, MartinProjectCleaner cleaner)
{
    public int Handle(CleanCommandOptions options)
    {
        ...
        if (options.DryRun)
            foreach (var directory in result.Plan.Directories)
                console.Out.WriteLine($"Would remove {directory}");
        else if (options.Verbose)
            foreach (var directory in result.Plan.Directories)
                console.Out.WriteLine($"Removed {directory}");
        if (!result.Success)
        {
            diagnostics.Render(result.Diagnostics);
            return (int)(result.Diagnostics.Any(diagnostic => diagnostic.Code == "MRT4513") ? MartinExitCode.ManifestInvalid : MartinExitCode.BuildFailed);
        }
        return (int)MartinExitCode.Success;
    }
