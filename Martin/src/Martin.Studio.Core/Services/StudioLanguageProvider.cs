...
public sealed class StudioLanguageProvider : IAsyncDisposable
{
    ...
    {
        if (_projectId is not {} id)
            throw new InvalidOperationException("No language project is open.");
        ...
        {
            _documentIds.Remove(mapping.Key);
            _studioIds.Remove(mapping.Value);
        }
    ...
    public async Task<DocumentId> OpenDocumentAsync(DocumentModel document, CancellationToken cancellationToken = default)
    {
        ...
        if (_documentIds.TryGetValue(document.Id, out var previous))
            _studioIds.Remove(previous);
        _documentIds[document.Id] = id;
        _studioIds[id] = document.Id;
        return id;
    }
                                                              ...
                                                              ImmutableArray<Martin.LanguageServices.TextChange> changes, CancellationToken cancellationToken = default)
    {
        ...
        return _languageWorkspace.ApplyDocumentChangesAsync(new Martin.LanguageServices.DocumentChange { DocumentId = id, PreviousVersion = new(previousVersion.Value), NewVersion = new(newVersion.Value), Changes = changes }, cancellationToken);
    }

    public async Task ReplaceDocumentTextAsync(DocumentModel document, CancellationToken cancellationToken = default)
    {
        if (!_documentIds.TryGetValue(document.Id, out var id))
        {
            await OpenDocumentAsync(document, cancellationToken);
            return;
        }
    ...
    {
        if (!_documentIds.Remove(studioId, out var id))
            return;
        _studioIds.Remove(id);
        if (Workspace.FindDocument(id) is not null)
            await _languageWorkspace.CloseDocumentAsync(id, cancellationToken);
    }

    public async Task RenameDocumentAsync(Guid studioId, string newPath, CancellationToken cancellationToken = default)
    {
        if (!_documentIds.TryGetValue(studioId, out var id))
            throw new KeyNotFoundException("The Studio document is not mapped.");
    ...
    void OnDiagnosticSnapshot(object? sender, DiagnosticSnapshotEventArgs e)
    {
        ...
        {
            if (!_studioIds.TryGetValue(set.DocumentId, out var id))
                continue;
            ...
            var diagnostics = set.Diagnostics.Select(d => new StudioDiagnostic(d.Code, ToStudioSeverity(d.Severity), d.Message, d.FilePath,
                                                                               new(d.LineSpan.Start.Line + 1, d.LineSpan.Start.Character + 1, d.LineSpan.End.Line + 1, d.LineSpan.End.Character + 1), d.Source, id, version))
                                  .ToImmutableArray();
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException(nameof(StudioLanguageProvider));
    }
    ...
    public async Task<object> HandleMonacoRequestAsync(string type, JsonElement payload, CancellationToken cancellationToken = default)
    {
        ...
        if (!_documentIds.TryGetValue(studioId, out var documentId))
            return Stale(MonacoResponseIdentity.Unknown(requestedVersion, modelVersion));
        ...
        if (document is null || project is null || document.Version.Value != requestedVersion)
            return Stale(MonacoResponseIdentity.From(workspace, project, document, modelVersion));
        var position = type is "language/requestFormatting" or "language/requestSemanticTokens" ? 0 : PositionConverter.Instance.ToOffset(document.SourceText, new(payload.GetProperty("line").GetInt32() - 1, payload.GetProperty("column").GetInt32() - 1));
    ...
    bool IsCurrent(MonacoResponseIdentity identity)
    {
        ...
        if (current.Id.Value.ToString() != identity.WorkspaceId || current.Version.Value != identity.WorkspaceVersion)
            return false;
        var project = current.Projects.FirstOrDefault(p => p.Id.Value.ToString() == identity.ProjectId);
        if (project is null || project.Version.Value != identity.ProjectVersion)
            return false;
    ...
    Task<LanguageResult<T>> ScheduleAsync<TRequest, T>(TRequest request, AnalysisPriority priority,
        ...
        where TRequest : LanguageRequest => _analysisScheduler.RunAsync(request, priority, async (context, ct) => (await operation(context.Workspace, request, ct).ConfigureAwait(false)).Value, cancellationToken);

                         static object Completion(LanguageResult<ImmutableArray<CompletionItem>> result,
                         LanguageDocumentSnapshot document,
                         MonacoResponseIdentity identity,
                         bool invalidated) => new { identity.WorkspaceId,
    ...
    static object Tokens(LanguageResult<ImmutableArray<ClassifiedSpan>> result, LanguageDocumentSnapshot document, MonacoResponseIdentity identity, bool invalidated)
    {
        ...
        {
            var range = PositionConverter.Instance.ToRange(document.SourceText, item.Span);
            if (range.Start.Line != range.End.Line)
                continue;
            var type = SemanticTokenType(item.Kind);
            var lineDelta = range.Start.Line - previousLine;
            var characterDelta = lineDelta == 0 ? range.Start.Character - previousCharacter : range.Start.Character;
            data.AddRange([lineDelta, characterDelta, range.End.Character - range.Start.Character, type, (int)item.Modifiers]);
    ...
    static int SemanticTokenType(ClassificationKind kind) => kind switch { ClassificationKind.Keyword => 0, ClassificationKind.Comment or ClassificationKind.DocumentationComment => 1, ClassificationKind.NumberLiteral => 2, ClassificationKind.StringLiteral => 3, ClassificationKind.Operator => 4, ClassificationKind.Type => 5, ClassificationKind.Struct => 6, ClassificationKind.Class => 7, ClassificationKind.Enum => 8, ClassificationKind.EnumCase => 9, ClassificationKind.Protocol => 10, ClassificationKind.Function => 11, ClassificationKind.Method => 12, ClassificationKind.Initializer => 13, ClassificationKind.Property => 14, ClassificationKind.Parameter => 15,
                                                                           _ => 16 };
    object Location(LanguageWorkspaceSnapshot workspace, DocumentId id, string path, TextSpan span)
    {
        var document = workspace.FindDocument(id);
        return new { documentId = FindStudioId(id), filePath = path, range = document is null ? new { startLine = 1, startColumn = 1, endLine = 1, endColumn = 1 } : Range(document, span) };
    }
    Guid? FindStudioId(DocumentId id) => _studioIds.TryGetValue(id, out var key) ? key : null;
    static object Range(LanguageDocumentSnapshot document, TextSpan span)
    {
        var r = PositionConverter.Instance.ToRange(document.SourceText, span);
        return new { startLine = r.Start.Line + 1, startColumn = r.Start.Character + 1, endLine = r.End.Line + 1, endColumn = r.End.Character + 1 };
    }
