...
public readonly record struct WorkspaceGeneration(long Value)
{
...
public sealed record EditorViewState
{
    ...
    public ImmutableArray<SelectionRange> Selections { get; init; } = [];
}
public sealed class DocumentModel
{
    ...
    public LineEndingKind LineEndings { get; set; }
    public bool IsDirty { get; set; }
    public bool IsReadOnly { get; set; }
    public bool IsDeleted { get; set; }
    public DateTimeOffset? LastDiskWriteTime { get; set; }
    public string? SavedContentHash { get; set; }
    public bool HasExternalChanges { get; set; }
    public DocumentVersion Version { get; set; } = new(0);
...
public sealed class StudioSemanticTokensChangedEventArgs(Guid documentId, DocumentVersion version) : EventArgs
{
    public Guid DocumentId { get; } = documentId;
    public DocumentVersion Version { get; } = version;
}
public sealed record OutputEntry(DateTimeOffset Timestamp, OutputChannel Channel, OutputSeverity Severity, string Message);

public enum StudioLogCategory
{
    Studio,
    Workspace,
    ProjectSystem,
...
public enum StudioRecoveryKind
{
    SettingsCorruption,
    SessionCorruption,
    WatcherOverflow,
    WebView2ProcessFailure,
    UnexpectedException
}
...
public sealed class ProjectTreeNode
{
    ...
    public ProjectNodeKind Kind { get; init; }
...
{
    public MartinProject? Project { get; internal set; }
    public WorkspaceGeneration Generation { get; internal set; }
    public ObservableCollection<DocumentModel> OpenDocuments { get; } = [];
    public DocumentModel? ActiveDocument { get; internal set; }
    public ObservableCollection<StudioDiagnostic> Diagnostics { get; } = [];
    public BuildState BuildState { get; internal set; }
    public ExecutionState ExecutionState { get; internal set; }
    public Martin.Build.BuildConfiguration ActiveBuildConfiguration { get; set; } = Martin.Build.BuildConfiguration.Debug;
    public ProjectTreeNode? ProjectTree { get; internal set; }
}
public sealed record CompilationSnapshot(Compilation Compilation, ImmutableArray<SnapshotSource> Sources, ImmutableDictionary<Guid, DocumentVersion> Versions, WorkspaceGeneration Generation)
{
    public ImmutableArray<StudioDiagnostic> Diagnostics { get; init; } = [];
    public bool HasSourceReadErrors => Diagnostics.Any(d => d.Severity == OutputSeverity.Error);
}
...
public enum ShutdownTransitionKind
{
    CloseProject,
    OpenProject,
    CreateProject,
...
public enum StudioTransitionStatus
{
    Succeeded,
    Cancelled,
    Failed
...
public sealed record StudioShutdownResult(StudioTransitionStatus Status, string? Message = null)
{
    public bool CanShutdown => Status == StudioTransitionStatus.Succeeded;
}
...
public sealed record StudioProjectCreationResult
{
...
public sealed record SaveAllResult(ImmutableArray<SaveDocumentResult> Results)
{
...
public enum DirtyProjectTransitionChoice
{
    Cancel,
    DiscardAndContinue,
    SaveAndContinue
...
public enum CleanExternalChangeChoice
{
    Reload,
    KeepCurrentView
}
public enum DirtyExternalChangeChoice
{
    Cancel,
    KeepEditorVersion,
    ReloadDiskVersion,
...
public interface IExternalChangePolicy
{
    Task<CleanExternalChangeChoice> ConfirmCleanChangeAsync(ExternalChange change, CancellationToken cancellationToken = default);
    Task<DirtyExternalChangeChoice> ConfirmDirtyChangeAsync(ExternalChange change, CancellationToken cancellationToken = default);
    Task<string?> GetSaveAsPathAsync(DocumentModel document, CancellationToken cancellationToken = default);
}
public interface IDirtyProjectTransitionPolicy
{
    Task<DirtyProjectTransitionChoice> ConfirmAsync(IReadOnlyList<DocumentModel> dirtyDocuments, CancellationToken cancellationToken = default);
}
