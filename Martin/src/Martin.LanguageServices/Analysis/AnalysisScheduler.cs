...
public sealed class AnalysisScheduler : IAnalysisScheduler
{
    ...
    public AnalysisScheduler(ILanguageWorkspace workspace, AnalysisSchedulerOptions? options = null)
    {
        ...
        _workers = Enumerable.Range(0, _options.MaximumConcurrency).Select(
                                               ...
                                               Func<LanguageAnalysisContext, CancellationToken, Task<T>> operation, CancellationToken cancellationToken = default)
    {
        ...
        lock (_gate)
        {
            ...
            {
                Metrics.Reject();
                return Task.FromResult(Result<T>(request, default!, cancelled: true));
            }
            ...
            var documentToken = _documents.GetOrAdd(request.DocumentId,
                                                    _ => new())
                                    .Token;
            var projectToken = _projects.GetOrAdd(request.ProjectId,
                                                  _ => new())
                                   .Token;
    ...
    public void CancelDocument(DocumentId id)
    {
        ...
        if (_diagnosticDelays.TryRemove(id, out var delay))
        {
            delay.Cancel();
            delay.Dispose();
        }
    ...
    public void CancelProject(ProjectId id)
    {
        ...
        if (_projectDelays.TryRemove(id, out var delay))
        {
            delay.Cancel();
            delay.Dispose();
        }
    ...
    public void ScheduleProjectAnalysis(ProjectId id, ProjectVersion version) => Debounce(
        ...
        async ct =>
        { await _projectConcurrency.WaitAsync(ct).ConfigureAwait(false); try { await InvokeProjectAnalysisAsync(id, version, ct).ConfigureAwait(false); } finally { _projectConcurrency.Release(); } });
    ...
    async Task WorkerAsync()
    {
        try
        {
            while (true)
            {
                ...
                lock (_gate) if (_pending.TryGetValue(item.Key, out var current) && ReferenceEquals(current, item))
    ...
    LanguageAnalysisContext? Resolve(LanguageRequest request)
    {
        ...
        if (snapshot.Id != request.WorkspaceId || snapshot.Version != request.WorkspaceVersion)
            return null;
    ...
    void Debounce<TKey>(ConcurrentDictionary<TKey, CancellationTokenSource> map, TKey key, TimeSpan delay, Func<CancellationToken, Task> action)
        where TKey : notnull
    {
        ThrowIfDisposed();
        ...
        _ = task.ContinueWith(t =>
                              { if (t.IsFaulted) _ = t.Exception; _backgroundTasks.TryRemove(t, out _); },
    ...
    static async Task ObserveDebounceAsync<TKey>(ConcurrentDictionary<TKey, CancellationTokenSource> map, TKey key, CancellationTokenSource cts, TimeSpan delay, Func<CancellationToken, Task> action)
        where TKey : notnull
    {
        try
        {
        ...
        {
            if (((ICollection<KeyValuePair<TKey, CancellationTokenSource>>)map).Remove(new(key, cts)))
                cts.Dispose();
        }
    }

    static void ReplaceToken<TKey>(ConcurrentDictionary<TKey, CancellationTokenSource> map, TKey key)
        where TKey : notnull
    {
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException(nameof(AnalysisScheduler));
    }
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        _shutdown.Cancel();
        foreach (var c in _documents.Values.Concat(_projects.Values).Concat(_diagnosticDelays.Values).Concat(_projectDelays.Values))
            c.Cancel();
    ...
    interface IWorkItem
    {
        ...
        Task ExecuteAsync(AnalysisScheduler owner, CancellationToken shutdown);
        void Cancel();
    }
                             ...
                             WorkKey key, CancellationToken caller, CancellationToken document, CancellationToken project) : IWorkItem
    {
        ...
        public WorkKey Key => key;
        public long EnqueuedTimestamp { get; } = Stopwatch.GetTimestamp();
        public Task<LanguageResult<T>> Task => _completion.Task;
        public async Task ExecuteAsync(AnalysisScheduler owner, CancellationToken shutdown)
        {
            ...
            try
            {
                ...
                if (context is null)
                {
                    owner.Metrics.MarkStale();
                    _completion.TrySetResult(Result<T>(request, default!, stale: true));
                    return;
                }
                var value = await operation(context, linked.Token).ConfigureAwait(false);
                if (owner.Resolve(request) is null)
                {
                    owner.Metrics.MarkStale();
                    _completion.TrySetResult(Result<T>(request, default!, stale: true));
                }
                ...
                {
                    owner.Metrics.Complete();
                    _completion.TrySetResult(Result(request, value));
                }
            }
            catch (OperationCanceledException) when (linked.IsCancellationRequested)
            {
                owner.Metrics.Cancel();
                _completion.TrySetResult(Result<T>(request, default!, cancelled: true));
            }
            catch (Exception ex)
            {
