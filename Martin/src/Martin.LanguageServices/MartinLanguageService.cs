...
{
 readonly LanguageAnalysisCache _cache=cache??new(); static readonly string[] Types=["Int","Double","String","Bool","Void"]; static readonly string[] Keywords=["func","let","var","if","else","while","return","true","false","nil","struct","class","init","self","mutating","protocol","enum","case","switch","default"];
 ...
 async Task<IAnalysisLease<ProjectAnalysis>> GetProjectAnalysisAsync(LanguageProjectSnapshot project,CancellationToken ct)
 {
  ...
  return await _cache.GetOrCreateProjectAsync(new(project.Id,project.Version),factoryCt=>
  {
   ...
   var trees=project.Documents.Select(d=>{factoryCt.ThrowIfCancellationRequested();return SyntaxTree.Parse(d.Text,d.FilePath,factoryCt);}).ToImmutableArray();
  ...
  },project.Documents.Sum(d=>d.Text.Length)*4L,ct).ConfigureAwait(false);
 }

 static IEnumerable<SyntaxNode> Flatten(SyntaxNode node)
 {
  yield return node;
  foreach(var child in node.GetChildren()) foreach(var descendant in Flatten(child)) yield return descendant;
 }
 ...
 public async Task<ImmutableArray<LanguageDiagnostic>> GetProjectSemanticDiagnosticsAsync(LanguageProjectSnapshot project,CancellationToken ct=default)
 {
  ...
  return lease.Value.Diagnostics
   ...
   .Select(d=>d with { DiagnosticSource=LanguageDiagnosticSource.LiveSemantic,Source="Martin.Live.Semantic" }).ToImmutableArray();
 }
 public LanguageWorkspaceSnapshot CreateWorkspaceFromProject(MartinProject project){var docs=project.SourceFiles.Select(f=>new LanguageDocumentSnapshot(DocumentId.CreateNew(),f,File.Exists(f)?File.ReadAllText(f):string.Empty,new(0))).ToImmutableArray();var p=new LanguageProjectSnapshot(ProjectId.CreateNew(),project.Manifest.Package.Name,project.RootDirectory,new(0),docs);return new(WorkspaceId.CreateNew(),new(0),[p]);}
 public async Task<DocumentAnalysis> AnalyzeDocumentAsync(LanguageWorkspaceSnapshot workspace,DocumentId documentId,CancellationToken ct=default)
 {
  ...
  try
  {
   foreach(var source in project.Documents)
   {
    ...
    leases.Add(await _cache.GetOrCreateDocumentAsync(key,_=>
    {
   ...
   return document with { Compilation=projectLease.Value.Compilation, Diagnostics=diagnostics,
 ...
 public async Task<VersionedResponse<ImmutableArray<LanguageDiagnostic>>> GetDiagnosticsAsync(LanguageWorkspaceSnapshot w,VersionedRequest<object?> request,CancellationToken ct=default){var current=w.FindDocument(request.DocumentId); if(current is null||current.Version!=request.Version)return new(request.DocumentId,request.Version,true,[]); var a=await AnalyzeDocumentAsync(w,request.DocumentId,ct); return new(request.DocumentId,request.Version,false,a.Diagnostics);}
 public ImmutableArray<CompletionItem> Complete(LanguageWorkspaceSnapshot w,DocumentId id,int position)
 { var d=w.FindDocument(id); if(d is null)return []; var p=w.Projects.First(x=>x.Documents.Any(y=>y.Id==id)); return WithProjectAnalysis(p, a => SemanticCompletionProvider.Complete(w,p,a,d,position,200,CancellationToken.None)); }
 ...
 {
  ct.ThrowIfCancellationRequested(); var p=w.FindProject(request.ProjectId); var d=w.FindDocument(request.DocumentId);
 ...
 public HoverInfo? Hover(LanguageWorkspaceSnapshot w,DocumentId id,int position){var doc=w.FindDocument(id);if(doc is null)return null;var project=w.Projects.FirstOrDefault(p=>p.Documents.Any(d=>d.Id==id));return project is null?null:WithProjectAnalysis(project,a=>SemanticHoverProvider.Get(project,a,doc,position,CancellationToken.None));}
 ...
 {
  ct.ThrowIfCancellationRequested();var project=w.FindProject(request.ProjectId);var document=w.FindDocument(request.DocumentId);
 ...
 { var project=w.Projects.FirstOrDefault(p=>p.Documents.Any(d=>d.Id==id)); if(project is null)return null; var context=WithProjectAnalysis(project,a=>SemanticNavigationProvider.Build(a,CancellationToken.None)); return SemanticNavigationProvider.Definitions(context,SemanticNavigationProvider.Resolve(context,id,position)).FirstOrDefault(); }
 public ImmutableArray<ReferenceLocation> FindReferences(LanguageWorkspaceSnapshot w,string symbol)
 { foreach(var project in w.Projects){var context=WithProjectAnalysis(project,a=>SemanticNavigationProvider.Build(a,CancellationToken.None)); var ids=context.Declarations.ByName.TryGetValue(symbol,out var found)?found:[]; if(ids.Length==1)return SemanticNavigationProvider.References(context,ids[0],default,true);} return []; }
 ...
 { ct.ThrowIfCancellationRequested(); var project=w.FindProject(request.ProjectId);var document=w.FindDocument(request.DocumentId);var stale=IsStale(w,project,document,request);var value=ImmutableArray<DefinitionLocation>.Empty;if(!stale){var context=await WithProjectAnalysisAsync(project!,a=>SemanticNavigationProvider.Build(a,ct),ct);value=SemanticNavigationProvider.Definitions(context,SemanticNavigationProvider.Resolve(context,request.DocumentId,request.Position));}return Result(request,value,stale); }
 ...
 { ct.ThrowIfCancellationRequested();var project=w.FindProject(request.ProjectId);var document=w.FindDocument(request.DocumentId);var stale=IsStale(w,project,document,request);var value=ImmutableArray<ReferenceLocation>.Empty;if(!stale){var context=await WithProjectAnalysisAsync(project!,a=>SemanticNavigationProvider.Build(a,ct),ct);value=SemanticNavigationProvider.References(context,SemanticNavigationProvider.Resolve(context,request.DocumentId,request.Position),request.DocumentId,request.IncludeDeclaration);}return Result(request,value,stale); }
 public SignatureHelp? GetSignatureHelp(LanguageWorkspaceSnapshot w,DocumentId id,int position){var doc=w.FindDocument(id);var project=w.Projects.FirstOrDefault(p=>p.Documents.Any(d=>d.Id==id));return doc is null||project is null?null:WithProjectAnalysis(project,a=>SemanticSignatureHelpProvider.Get(project,a,doc,position,CancellationToken.None));}
 ...
 {ct.ThrowIfCancellationRequested();var p=w.FindProject(request.ProjectId);var d=w.FindDocument(request.DocumentId);var stale=IsStale(w,p,d,request);var value=stale?null:await WithProjectAnalysisAsync(p!,a=>SemanticSignatureHelpProvider.Get(p!,a,d!,request.Position,ct),ct);return Result(request,value,stale);}
 ...
 {var p=w.FindProject(request.ProjectId);var d=w.FindDocument(request.DocumentId);var stale=IsStale(w,p,d,request);if(stale)return Result(request,ImmutableArray<ClassifiedSpan>.Empty,true);var analysis=await AnalyzeDocumentAsync(w,request.DocumentId,ct);return Result(request,analysis.Classifications,false);}
 ...
 public string FormatDocument(string text){var result=GetFormattingEdits(text);return result.WasRefused?text:TextEditApplicator.Apply(text,result.Edits);}
 ...
 {ct.ThrowIfCancellationRequested();var p=w.FindProject(request.ProjectId);var d=w.FindDocument(request.DocumentId);var stale=IsStale(w,p,d,request);var value=stale?new FormattingResult():SafeFormattingService.Format(d!.Text,request.Options);return Task.FromResult(Result(request,value,stale));}
 ...
 static IEnumerable<ClassifiedSpan> TokenClassifications(SyntaxToken t){foreach(var tr in t.LeadingTrivia.Concat(t.TrailingTrivia).Where(x=>x.Kind is SyntaxKind.SingleLineCommentTrivia or SyntaxKind.MultiLineCommentTrivia or SyntaxKind.DocumentationCommentTrivia))yield return new(tr.Span,tr.Kind==SyntaxKind.DocumentationCommentTrivia?ClassificationKind.DocumentationComment:ClassificationKind.Comment); var kind=t.Kind switch{var k when SyntaxFacts.IsKeyword(k)=>ClassificationKind.Keyword,SyntaxKind.IdentifierToken=>ClassificationKind.UnresolvedIdentifier,SyntaxKind.IntegerLiteralToken or SyntaxKind.FloatingPointLiteralToken=>ClassificationKind.NumberLiteral,SyntaxKind.StringLiteralToken=>ClassificationKind.StringLiteral,var k when SyntaxFacts.GetBinaryOperatorPrecedence(k)>0||SyntaxFacts.GetUnaryOperatorPrecedence(k)>0||k is SyntaxKind.EqualToken or SyntaxKind.ArrowToken=>ClassificationKind.Operator,_=>ClassificationKind.Punctuation}; yield return new(t.Span,kind);}
 static void WriteComments(StringBuilder sb,IEnumerable<SyntaxTrivia> trivia,int indent){foreach(var tr in trivia.Where(IsCommentTrivia)){TrimLine(sb,indent); sb.Append(tr.Text); sb.AppendLine();}}
 static void WriteTrailingComments(StringBuilder sb,IEnumerable<SyntaxTrivia> trivia){foreach(var tr in trivia.Where(IsCommentTrivia)){if(sb.Length>0&&!char.IsWhiteSpace(sb[^1]))sb.Append(' '); sb.Append(tr.Text);}}
 ...
 static ImmutableArray<string> CollectSymbols(LanguageWorkspaceSnapshot w){var result=ImmutableArray.CreateBuilder<string>(); foreach(var d in w.Projects.SelectMany(p=>p.Documents)){var tree=SyntaxTree.Parse(d.Text,d.FilePath);var compilation=Compilation.Create(tree); _=compilation.BindProgram(); result.AddRange(compilation.GetSemanticModel(tree).GetDeclaredSymbols().Select(s=>s.Name));} return result.Distinct().ToImmutableArray();}
 static (int start,string text) WordAt(string text,int pos){if(text.Length==0)return(0,""); pos=Math.Clamp(pos,0,text.Length-1); while(pos>0&&!IsWord(text[pos])&&IsWord(text[pos-1]))pos--; if(!IsWord(text[pos]))return(pos,""); var s=pos; while(s>0&&IsWord(text[s-1]))s--; var e=pos; while(e<text.Length&&IsWord(text[e]))e++; return(s,text[s..e]); static bool IsWord(char c)=>char.IsLetterOrDigit(c)||c=='_';}
 ...
 static void TrimLine(StringBuilder sb,int indent){if(sb.Length>0&&!sb.ToString().EndsWith(Environment.NewLine))sb.AppendLine(); sb.Append(new string(' ',indent*4));}
 static void EnsureIndent(StringBuilder sb,int indent){if(sb.Length==0||sb.ToString().EndsWith(Environment.NewLine))sb.Append(new string(' ',indent*4));}
}
