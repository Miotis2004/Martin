...
public sealed class ReferenceIndexBuilder
{
                                ...
                                IEnumerable<DocumentId>? documents, CancellationToken cancellationToken = default)
    {
        ...
        var declarations = analysis.SemanticModels.Values.SelectMany(model => model.GetDeclaredSymbols()
                                                                                  .Select(symbol => (Model: model, Symbol: symbol)))
                               .DistinctBy(x => x.Symbol, ReferenceEqualityComparer.Instance)
                               .ToArray();
        ...
        SymbolId Id(Symbol symbol)
        {
            ...
            if (ids.TryGetValue(symbol, out var existing))
                return existing;
            var owner = owners.TryGetValue(symbol, out var containing) ? Id(containing) : (SymbolId?)null;
            var discriminator = symbol is LocalVariableSymbol && symbol.DeclarationLocation is
            {
            }
            location
                ? $"{(pathMap.TryGetValue(Canonical(location.FilePath), out var documentId) ? documentId.Value.ToString("N") : string.Empty)}:{location.Span.Start}:{location.Span.Length}"
                : null;
        ...
        foreach (var documentId in requested.OrderBy(id => id.Value))
        {
            ...
            if (!documentMap.TryGetValue(documentId, out var document) ||
                !analysis.SemanticModels.TryGetValue(documentId, out var model))
                continue;
        ...
        var ordered = results.Distinct().OrderBy(r => r.FilePath, PathComparer).ThenBy(r => r.Span.Start).ThenBy(r => r.Span.Length).ThenBy(r => r.Kind).ToArray();
    ...
    static Dictionary<Symbol, Symbol> BuildOwners(IEnumerable<(SemanticModel Model, Symbol Symbol)> declarations)
    {
        ...
        {
            if (symbol is MemberSymbol member && member.ContainingType is Symbol type)
                map[symbol] = type;
            if (symbol is TypeParameterSymbol typeParameter)
                map[symbol] = typeParameter.ContainingSymbol;
            ...
            if (symbol is LocalVariableSymbol local && local.DeclarationLocation is {} location &&
                model.GetEnclosingSymbol(location.Span.Start) is {} enclosing)
                map[local] = enclosing;
        }
        return map;
    }
