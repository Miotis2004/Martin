...
public sealed class DeclarationIndexBuilder
{
    public DeclarationIndex Build(LanguageProjectSnapshot project, ProjectAnalysis analysis)
    {
        ...
        {
            if (ids.TryGetValue(symbol, out var id)) return id;
        ...
        var ordered = descriptors.OrderBy(d => d.Definition.FilePath, PathComparer)
            .ThenBy(d => d.Definition.Span.Start).ThenBy(d => d.Id.Value, StringComparer.Ordinal).ToArray();
        var byId = ordered.ToImmutableDictionary(d => d.Id);
        var byName = ordered.GroupBy(d => d.Name, StringComparer.Ordinal).ToImmutableDictionary(
            group => group.Key,
            group => group.Select(d => d.Id).OrderBy(id => id.Value, StringComparer.Ordinal).ToImmutableArray(),
            StringComparer.Ordinal);
        var byDocument = ordered.GroupBy(d => d.Definition.DocumentId).ToImmutableDictionary(
            group => group.Key,
            group => group.Select(d => d.Id).ToImmutableArray());
        ...
        foreach (var entry in declarations)
        {
            ...
            switch (entry.Symbol)
            {
                case FunctionSymbol function:
                    foreach (var parameter in function.Parameters) map[parameter] = function;
                    foreach (var parameter in function.TypeParameters) map[parameter] = function;
                    break;
                case MethodSymbol method:
                    foreach (var parameter in method.Parameters) map[parameter] = method;
                    break;
                case InitializerSymbol initializer:
                    foreach (var parameter in initializer.Parameters) map[parameter] = initializer;
                    break;
                case EnumCaseSymbol enumCase:
                    foreach (var parameter in enumCase.AssociatedValues) map[parameter] = enumCase;
                    break;
                case LocalVariableSymbol local when local.DeclarationLocation is { } location:
                    ...
                    if (enclosing is not null) map[local] = enclosing;
                    break;
            }
        }
        return map;
        ...
        out DefinitionLocation definition, out ImmutableArray<DefinitionLocation> additional)
    {
        var locations = symbol.Locations.Select(location =>
        {
            ...
            return path is not null && documents.TryGetValue(Canonical(path), out var document)
                ...
                : null;
        }).OfType<DefinitionLocation>().Distinct().OrderBy(l => l.FilePath, PathComparer).ThenBy(l => l.Span.Start).ToArray();
        if (locations.Length == 0) { definition = null!; additional = []; return false; }
    ...
    static string DeclarationKey(Symbol symbol, Dictionary<string, LanguageDocumentSnapshot> documents)
    {
        if (symbol.DeclarationLocation is not { } location || location.FilePath is null ||
            !documents.TryGetValue(Canonical(location.FilePath), out var document)) return string.Empty;
