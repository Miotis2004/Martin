...
internal static class SafeFormattingService
{
    ...
    public static FormattingResult Format(string source, FormattingOptions? options = null)
    {
        ...
        var items = Tokens(tree.Root)
                        ...
                        .SelectMany(t => t.LeadingTrivia.Concat(t.TrailingTrivia)
                                             .Where(IsPreservedTrivia)
                                             .Select(x => new Item(x.Kind, x.Text, x.Span.Start, true))
                        ...
                        .DistinctBy(x => (x.Start, x.Text, x.Kind))
                        .OrderBy(x => x.Start)
                        .ToArray();
    ...
    private static string Render(Item[] items, FormattingOptions options, string nl)
    {
        ...
        var indent = 0;
        var activeCaseBodies = 0;
        var lineStart = true;
        Item? previous = null;
        void NewLine()
        {
            while (b.Length > 0 && (b[^1] == ' ' || b[^1] == '\t'))
                b.Length--;
            if (b.Length == 0 || !EndsWith(b, nl))
                b.Append(nl);
            lineStart = true;
            previous = null;
        }
        void Indent()
        {
            if (!lineStart)
                return;
            b.Append(options.UseTabs ? new string('\t', indent) : new string(' ', indent * options.IndentSize));
            lineStart = false;
        }
        foreach (var item in items)
        {
            if ((item.IsSwitchCaseStart || item.ClosesSwitch) && activeCaseBodies > 0)
            {
                indent--;
                activeCaseBodies--;
            }
            if (item.BreakBefore && !lineStart)
                NewLine();
            if (item.IsTrivia)
            {
                if (!lineStart)
                ...
                if (item.Kind is SyntaxKind.SingleLineCommentTrivia or SyntaxKind.DocumentationCommentTrivia || item.Text.Contains('\n') || item.Text.Contains('\r'))
                    NewLine();
                else if (item.Kind == SyntaxKind.MultiLineCommentTrivia && b.Length > 0 && b[^1] is not(' ' or '\t'))
                    b.Append(' ');
                previous = null;
                continue;
            }
            if (item.Kind == SyntaxKind.CloseBraceToken)
            {
            ...
            Indent();
            if (NeedsSpace(previous, item))
                b.Append(' ');
            b.Append(item.Text);
            if (item.Kind == SyntaxKind.OpenBraceToken)
            {
                indent++;
                NewLine();
            }
            else if (item.Kind is SyntaxKind.CloseBraceToken or SyntaxKind.SemicolonToken)
                NewLine();
            else if (item.Kind == SyntaxKind.CommaToken)
                b.Append(' ');
            if (item.BreakAfter)
            {
                indent++;
        ...
        while (b.Length > 0 && char.IsWhiteSpace(b[^1]))
            b.Length--;
        if (options.InsertFinalNewLine && b.Length > 0)
            b.Append(nl);
        return b.ToString();
    }
    ...
    private static bool NeedsSpace(Item? previous, Item current)
    {
        ...
        if (current.IsGenericDelimiter || previous.Value.IsGenericDelimiter && left == SyntaxKind.LessToken)
            return false;
        if (previous.Value.IsGenericDelimiter && left == SyntaxKind.GreaterToken &&
            ...
            return false;
        if (right == SyntaxKind.DotToken && left is SyntaxKind.CaseKeyword or SyntaxKind.CatchKeyword)
            return true;
        if (right is SyntaxKind.CommaToken or SyntaxKind.ColonToken or SyntaxKind.DotToken or SyntaxKind.SemicolonToken or SyntaxKind.CloseParenthesisToken or SyntaxKind.CloseBracketToken or SyntaxKind.QuestionToken)
            return false;
        if (left is SyntaxKind.OpenParenthesisToken or SyntaxKind.OpenBracketToken or SyntaxKind.DotToken)
            return false;
        if (right == SyntaxKind.OpenParenthesisToken && left is SyntaxKind.IdentifierToken or SyntaxKind.SelfKeyword)
            return false;
    ...
    private static IEnumerable<SyntaxToken> Tokens(SyntaxNode node)
    {
        ...
        foreach (var child in node.GetChildren())
            foreach (var descendantToken in Tokens(child))
    ...
    private static bool EndsWith(StringBuilder b, string value)
    {
        if (b.Length < value.Length)
            return false;
        for (var i = 0; i < value.Length; i++)
            if (b[b.Length - value.Length + i] != value[i])
                return false;
        return true;
    }
...
public static class TextEditApplicator
{
    public static void Validate(string source, ImmutableArray<TextEdit> edits)
    {
        ArgumentNullException.ThrowIfNull(source);
        if (edits.IsDefault)
            throw new ArgumentException("Edits must be initialized.", nameof(edits));
        var end = 0;
        foreach (var edit in edits)
        {
            if (edit.NewText is null || edit.Span.Start < end || edit.Span.Start < 0 || edit.Span.Length < 0 || edit.Span.End > source.Length)
                throw new ArgumentException("Edits must be sorted, non-overlapping, and within the source.", nameof(edits));
            end = edit.Span.End;
        }
    }
    public static string Apply(string source, ImmutableArray<TextEdit> edits)
    {
        ...
        {
            var e = edits[i];
            b.Remove(e.Span.Start, e.Span.Length).Insert(e.Span.Start, e.NewText);
        }
        return b.ToString();
    }
}
