...
internal static class SemanticSignatureHelpProvider
{
                                     ...
                                     CancellationToken cancellationToken)
    {
        ...
        var activeParameter = invocation.GetChildren().OfType<SyntaxToken>().Count(t => t.Kind == SyntaxKind.CommaToken && t.Span.Start < position);
        var activeSignature = 0;
        for (var i = 0; i < signatures.Length; i++)
            if (activeParameter < signatures[i].Parameters.Length)
            {
                activeSignature = i;
                break;
            }
    ...
    {
        var pattern = DescendantsAndSelf(model.SyntaxTree.Root).OfType<EnumCasePatternSyntax>().Where(p => p.Arguments is not null && p.Arguments.FullSpan.Start <= position && position <= p.Arguments.FullSpan.End).OrderBy(p => p.FullSpan.Length).FirstOrDefault();
        if (pattern is null || model.GetPatternInfo(pattern)?.EnumCase is not {} enumCase)
            return null;
        var identity = new SymbolIdentityFactory(project.Id);
    ...
    private static SignatureInformation Information(Symbol symbol, SymbolIdentityFactory identity)
    {
        var parameters = Parameters(symbol).Select(p => new ParameterInformation(
                                                       p.Label is null or "_" ? p.Name : p.Label + ": " + p.Name, TypeName(p.Type)))
                             .ToImmutableArray();
    ...
    {
        var supplied = invocation.GetChildren().Where(n => n.Kind == SyntaxKind.Argument && n.Span.Start < position).Select(n => n.GetChildren().OfType<SyntaxToken>().Take(2).ToArray()).Select((tokens, index) => (index, label: tokens.Length == 2 && tokens[1].Kind == SyntaxKind.ColonToken ? tokens[0].Text : null));
    ...
    private static string CalleeName(SyntaxNode call) => call.GetChildren().FirstOrDefault() is
    {
    }
    callee? DescendantsAndSelf(callee).OfType<SyntaxToken>().LastOrDefault(t => t.Kind == SyntaxKind.IdentifierToken)?.Text ?? "" : "";
    ...
    private static bool EnclosesCaret(SyntaxNode n, int p)
    {
        var open = OpenParenthesisPosition(n);
        var close = n.GetChildren().OfType<SyntaxToken>().LastOrDefault(t => t.Kind == SyntaxKind.CloseParenthesisToken);
        return open < p && (close is null || close.IsMissing || p <= close.Span.Start);
    }
    private static bool InTriviaOrString(SemanticModel model, int p)
    {
        var token = model.FindToken(p == model.SyntaxTree.Text.Length && p > 0 ? p - 1 : p, true);
        return token is not null && (token.Kind == SyntaxKind.StringLiteralToken && token.Span.Start < p && p < token.Span.End || token.LeadingTrivia.Concat(token.TrailingTrivia).Any(t => t.Span.Start <= p && p <= t.Span.End && t.Kind is SyntaxKind.SingleLineCommentTrivia or SyntaxKind.MultiLineCommentTrivia or SyntaxKind.DocumentationCommentTrivia));
    }
    private static IEnumerable<SyntaxNode> DescendantsAndSelf(SyntaxNode n)
    {
        yield return n;
