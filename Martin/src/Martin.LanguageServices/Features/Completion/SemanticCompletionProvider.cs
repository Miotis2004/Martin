...
internal static class SemanticCompletionProvider
{
        ...
        ProjectAnalysis analysis, LanguageDocumentSnapshot document, int position, int maximumResults, CancellationToken cancellationToken)
    {
        ...
        if (!analysis.SemanticModels.TryGetValue(document.Id, out var model)) return [];
        if (IsInCommentOrString(model, position)) return [];
        ...
        IEnumerable<Symbol> symbols = context.MemberReceiver is { } receiver
            ? model.GetTypeInfo(receiver) is { } receiverType ? model.LookupMembers(receiverType, position) : []
            : model.LookupSymbols(position);
        foreach (var symbol in symbols)
        {
            cancellationToken.ThrowIfCancellationRequested();
            if (!ValidInContext(symbol, context)) continue;
        ...
        if (context.MemberReceiver is null)
        {
            var keywords = context.IsType ? Array.Empty<string>() : context.IsTopLevel ? TopLevelKeywords :
                context.IsExpression ? ExpressionKeywords.Concat(StatementKeywords) : StatementKeywords;
            foreach (var keyword in keywords.Distinct(StringComparer.Ordinal))
            {
                if (keyword == "throw" && !CanThrowHere(model, position)) continue;
    ...
    {
        var declaration = Descendants(model.SyntaxTree.Root).FirstOrDefault(node =>
            (node.Kind is SyntaxKind.StructDeclaration or SyntaxKind.ClassDeclaration or SyntaxKind.EnumDeclaration) &&
            node.FullSpan.Start <= position && position <= node.FullSpan.End);
        ...
        var colon = declaration.GetChildren().SelectMany(Descendants).OfType<SyntaxToken>()
            .FirstOrDefault(t => t.Kind == SyntaxKind.ColonToken && (openBrace is null || t.Span.Start < openBrace.Span.Start));
        if (colon is null || position <= colon.Span.End || openBrace is not null && position > openBrace.Span.Start) return false;

        var listed = declaration.GetChildren().Where(n => n.Kind == SyntaxKind.ProtocolConformanceClause)
            .SelectMany(Descendants).OfType<SyntaxToken>().Where(t => t.Kind == SyntaxKind.IdentifierToken && t.Span.End <= position)
            .Select(t => t.Text).Where(name => !name.Equals(prefix, StringComparison.Ordinal)).ToHashSet(StringComparer.Ordinal);
        foreach (var protocol in model.LookupSymbols(position).OfType<ProtocolTypeSymbol>()
                     .Where(p => !listed.Contains(p.Name)).DistinctBy(p => p.Name))
            AddSymbol(candidates, protocol, replacement, prefix, null, identity);
        return true;
    }
    ...
    private static ImmutableArray<CompletionItem> Finish(List<Candidate> candidates, int maximumResults) => candidates
            .GroupBy(c => (c.Item.Label, c.Item.TextEdit.NewText), StringTupleComparer.Instance).Select(g => g.MinBy(c => c.Rank)!)
            .OrderBy(c => c.Rank).ThenBy(c => c.Item.SortText, StringComparer.Ordinal).ThenBy(c => c.Item.Label, StringComparer.Ordinal)
            .Take(Math.Clamp(maximumResults, 1, 1000)).Select(c => c.Item).ToImmutableArray();
        ...
        int position, TextSpan replacement, string prefix, List<Candidate> candidates, CancellationToken cancellationToken)
    {
        ...
        if (catchNode is null) return false;
        ...
        if (catchOffset < 0 || braceOffset >= 0) return false;
        var fragment = before[(catchOffset + 5)..].TrimStart();
        if (fragment.Length > 0 && !fragment.StartsWith('.') && !fragment.StartsWith('_') && !fragment.StartsWith("let", StringComparison.Ordinal) && !IsIdentifier(fragment[0])) return false;

        cancellationToken.ThrowIfCancellationRequested();
        var errorType = model.GetCatchInfo(catchNode)?.DeclaredErrorType ?? model.GetCatchInfo(catchNode)?.Effect.ErrorType;
        if (errorType is null) return false;
        ...
        EnumCaseSymbol enumCase, int rank, string detail)
    {
        ...
        if (replacement.Start > 0 && text[replacement.Start - 1] == '.' && insertion.StartsWith('.')) insertion = insertion[1..];
    ...
    private static bool TryPatternContext(SemanticModel model, ProjectAnalysis analysis, LanguageDocumentSnapshot document,
        int position, TextSpan replacement, string prefix, List<Candidate> candidates, CancellationToken cancellationToken)
    {
        var switchNode = Descendants(model.SyntaxTree.Root).OfType<StatementSyntax>()
            .Where(n => n.Kind == SyntaxKind.SwitchStatement && n.FullSpan.Start <= position && position <= n.FullSpan.End)
            .OrderBy(n => n.FullSpan.Length).FirstOrDefault();
        if (switchNode is null) return false;
        ...
        if (caseOffset < 0 || before.LastIndexOf(':') > caseOffset + 4) return false;
        ...
        if (fragment.Length > 0 && !fragment.StartsWith('.') && !fragment.StartsWith("nil", StringComparison.Ordinal) &&
            ...
            !fragment.StartsWith('_') && !IsIdentifier(fragment[0])) return false;
        ...
        if (inputType is null) return false;
        ...
        void AddPattern(string label, string insertion, CompletionItemKind kind, int rank, string detail, bool snippet = false)
            => candidates.Add(Create(label, replacement.Start > 0 && document.Text[replacement.Start - 1] == '.' && insertion.StartsWith('.') ? insertion[1..] : insertion,
        ...
        else
        {
            ...
            if (!Cases(inputType).Any()) AddPattern("_", "_", CompletionItemKind.Keyword, 15, "wildcard pattern");
        }
        return true;
    }
    ...
    private static string CaseSnippet(EnumCaseSymbol c) => '.' + c.Name + (c.HasAssociatedValues
        ? "(" + string.Join(", ", c.AssociatedValues.Select((p, i) =>
            $"let ${{{i + 1}:{BindingName(p, c.AssociatedValues.Length)}}}")) + ")" : string.Empty);
    ...
    private static class MissingPatternWitnessSnippetRenderer
    {
        ...
        private static string Render(MissingPatternWitness witness, ref int placeholder)
        {
            ...
            if (witness.Arguments.IsEmpty) return witness.Constructor.Name;
    ...
    {
        var kind = Kind(symbol); var insertion = symbol.Name;
        if (symbol is InitializerSymbol) insertion = "init";
        ...
        var locality = symbol.Kind switch { SymbolKind.LocalVariable => 10, SymbolKind.Parameter => 12, SymbolKind.Property or SymbolKind.Method => 18, _ => 30 };
        ...
        var item = Create(symbol.Name, insertion, kind, span, prefix, rank, signature).Item with
        {
            SymbolId = identity.Create(symbol, declarationDiscriminator: symbol.DeclarationLocation?.Span.Start.ToString()),
            IsRecommended = expectedMatch,
            CommitCharacters = symbol is FunctionSymbol or MethodSymbol or InitializerSymbol ? ['('] : symbol is MemberSymbol ? ['.'] : []
        };
    ...
    {
        var used = invocation.GetChildren().Where(n => n.Kind == SyntaxKind.Argument)
            .Select(n => n.GetChildren().OfType<SyntaxToken>().FirstOrDefault())
            .Where(t => t?.Kind == SyntaxKind.IdentifierToken && t.Span.Start < position).Select(t => t!.Text).ToHashSet(StringComparer.Ordinal);
    ...
    private static bool IsThrowsTypeContext(SemanticModel model, int position)
    {
        ...
        if (throws is null) return false;
    ...
    private static bool IsInCommentOrString(SemanticModel model, int position)
    {
        ...
        if (token.Kind == SyntaxKind.StringLiteralToken && token.Span.Start < position && position < token.Span.End) return true;
        return token.LeadingTrivia.Concat(token.TrailingTrivia).Any(t => t.Span.Start <= position && position <= t.Span.End &&
            t.Kind is SyntaxKind.SingleLineCommentTrivia or SyntaxKind.MultiLineCommentTrivia or SyntaxKind.DocumentationCommentTrivia);
    }

    private static TextSpan ReplacementSpan(string text, int position)
    {
        var start = position; while (start > 0 && IsIdentifier(text[start - 1])) start--;
        var end = position; while (end < text.Length && IsIdentifier(text[end])) end++;
